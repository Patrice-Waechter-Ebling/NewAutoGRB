...
55      woups "frmInventaireMec", "cmdImprimer_Click", Err, Erl
End Sub
...
20          Call g_connData.Execute("DELETE * FROM GRB_InventaireMecModif WHERE NoEnreg = " & lvwHistorique.SelectedItem.Tag)
...
60      woups "frmInventaireMec", "lvwHistorique_KeyDown", Err, Erl
End Sub
...
45      Call rstInv.Open("SELECT * FROM GRB_InventaireMec WHERE NoItem = '" & Replace(sPiece, "'", "''") & "'", g_connData, adOpenDynamic, adLockOptimistic)
...
75      woups "frmInventaireMec", "ModifierInventaire", Err, Erl
End Sub
...
115     Call rstHist.Open("SELECT * FROM GRB_InventaireMecModif WHERE " & sWhere & " ORDER BY NoEnreg DESC", g_connData, adOpenDynamic, adLockOptimistic)
  
        'Il faut vider le ListView avant de le remplir
120     Call lvwHistorique.ListItems.Clear
  
        'Tant que ce n'est pas la fin des enregistrements
125     Do While Not rstHist.EOF
          'On l'ajoute
130       Set itmHist = lvwHistorique.ListItems.Add
    
          'Date
135       itmHist.Text = rstHist.Fields("Date")
140       itmHist.Tag = rstHist.Fields("NoEnreg")
    
          'User
145       itmHist.SubItems(I_HIST_USER) = rstHist.Fields("User")
    
          'IDProjet
150       If Not IsNull(rstHist.Fields("IDProjet")) Then
155         If Trim(rstHist.Fields("IDProjet")) <> "" Then
160           itmHist.SubItems(I_HIST_PROJET) = rstHist.Fields("IDProjet")
165         Else
170           itmHist.SubItems(I_HIST_PROJET) = "N/A"
175         End If
180       Else
185         itmHist.SubItems(I_HIST_PROJET) = "N/A"
190       End If
    
          'No Item
195       itmHist.SubItems(I_HIST_PIECE) = rstHist.Fields("NoItem")
    
          'Quantité
200       itmHist.SubItems(I_HIST_QUANTITE) = rstHist.Fields("Quantité")
    
205       Call rstHist.MoveNext
210     Loop
  
215     Call rstHist.Close
220     Set rstHist = Nothing

225     Exit Sub

AfficherErreur:

230     woups "frmInventaireMec", "RemplirListViewHistorique", Err, Erl
End Sub
...
70      woups "frmInventaireMec", "chkChoix_Click", Err, Erl
End Sub
...
105     woups "frmInventaireMec", "cmdAfficherHistorique_Click", Err, Erl
End Sub
...
20      woups "frmInventaireMec", "cmdFermer_Click", Err, Erl
End Sub
...
20      woups "frmInventaireMec", "Form_Load", Err, Erl
End Sub
...
60      woups "frmInventaireMec", "lvwHistorique_ColumnClick", Err, Erl
End Sub
...
35      woups "frmInventaireMec", "mskDateDebut_GotFocus", Err, Erl
End Sub
...
65      woups "frmInventaireMec", "mskDateDebut_LostFocus", Err, Erl
End Sub
...
35      woups "frmInventaireMec", "mskDateFin_GotFocus", Err, Erl
End Sub
...
65      woups "frmInventaireMec", "mskDateFin_LostFocus", Err, Erl
End Sub
...
25      Call rstInv.Open("SELECT NoItem, QuantitéStock, Description, Manufacturier, Localisation FROM GRB_InventaireMec ORDER BY Localisation, NoItem", g_connData, adOpenDynamic, adLockOptimistic)
...
40      DR_ListeInventaire.Sections("Section4").Controls("lblTitre").Caption = "Inventaire Mécanique"
...
65      woups "frmInventaireMec", "ImprimerAjustementInventaire", Err, Erl
End Sub
...
30      Call rstInv.Open("SELECT NoItem, Description, Manufacturier, [Prix Liste], Escompte, [Prix Net], Localisation, QuantitéStock, [Prix Net] * QuantitéStock As Total FROM GRB_InventaireMec WHERE QuantitéStock <> '0' ORDER BY NoItem", g_connData, adOpenDynamic, adLockOptimistic)
        
35      Set DR_Inventaire.DataSource = rstInv
    
40      Call rstTotal.Open("SELECT SUM([Prix net] * QuantitéStock) As Total FROM GRB_InventaireMec", g_connData, adOpenDynamic, adLockOptimistic)
...
55      DR_Inventaire.Sections("Section4").Controls("lblTitre").Caption = "Inventaire Mécanique"
...
95      woups "frmInventaireMec", "ImprimerValeurComptable", Err, Erl
End Sub
Private Function ExporterAjustementInventaire()
      On Error GoTo AfficherErreur
        ...
        Call rstInv.Open("SELECT NoItem, QuantitéStock, Description, Manufacturier, Localisation FROM GRB_InventaireMec ORDER BY Localisation, NoItem", g_connData, adOpenDynamic, adLockOptimistic)
        r = 1
...
65      woups "frmInventaireElec", "cmdExport_Click", Err, Erl
End Function
Private Function ExporterValeurComptable()
        ...
        Call rstInv.Open("SELECT NoItem, Description, Manufacturier, [Prix Liste], Escompte, [Prix Net], Localisation, QuantitéStock, [Prix Net] * QuantitéStock As Total FROM GRB_InventaireMec WHERE QuantitéStock <> '0' ORDER BY NoItem", g_connData, adOpenDynamic, adLockOptimistic)
        
        Set DR_Inventaire.DataSource = rstInv
    
        Call rstTotal.Open("SELECT SUM([Prix net] * QuantitéStock) As Total FROM GRB_InventaireMec", g_connData, adOpenDynamic, adLockOptimistic)
